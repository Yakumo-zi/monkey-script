...
func TestErrorHandling(t *testing.T) {
	...
	for idx, tt := range tests {
		...
		if !ok {
			t.Errorf("no error object returned. got=%T (%+v),idx=%d", evaluated, evaluated, idx)
			continue
		}
...
func TestStringLiteral(t *testing.T) {
	input := `"hello world!"`
	evaluated := testEval(input)
	str, ok := evaluated.(*object.StringObject)
	if !ok {
...
func TestBuiltinFunctions(t *testing.T) {
	...
	for idx, tt := range tests {
		...
		case []int64:
			if !testArrayObject(t, evaluated, expected) {
				t.Errorf("wrong array object. got=%+v", evaluated)
			}
		case nil:
			if !testNullObject(t, evaluated) {
				t.Errorf("wrong null object. got=%+v", evaluated)
			}
		}
	}
}
...
func TestArrayLiterals(t *testing.T) {
	input := "[1, 2*2, 3+3]"
	evaluated := testEval(input)
	result, ok := evaluated.(*object.ArrayObject)
	if !ok {
...
func TestHashLiterals(t *testing.T) {
	input := `let two = "two";
  {
	...
	evaluated := testEval(input)
	result, ok := evaluated.(*object.HashObject)
	if !ok {
		t.Fatalf("object is not Hash. got=%T (%+v)", evaluated, evaluated)
	}
	expected := map[object.HashKey]int64{
		(&object.StringObject{Value: "one"}).HashKey():   1,
		(&object.StringObject{Value: "two"}).HashKey():   2,
		(&object.StringObject{Value: "three"}).HashKey(): 3,
...
func TestHashIndexExpressions(t *testing.T) {
	...
	for _, tt := range tests {
		...
		if isError(evaluated) {
			t.Errorf("error returned: %s", evaluated.Inspect())
		}
		switch expected := tt.expected.(type) {
		case int:
			testIntegerObject(t, evaluated, int64(expected))
		case nil:
			testNullObject(t, evaluated)
		default:
			t.Errorf("unhandled type. got=%T", tt.expected)
		}
...
func testArrayObject(t *testing.T, obj object.Object, expected []int64) bool {
	result, ok := obj.(*object.ArrayObject)
	if !ok {
