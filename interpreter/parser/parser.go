...
func (p *Parser) parseHashLiteral() ast.Expression {
	...
	for !p.peekTokenIs(token.RBRACE) {
		...
		if !p.peekTokenIs(token.RBRACE) && !p.expectToken(token.COMMA) {
			return nil
		}
	}
...
func (p *Parser) registerPrefix(ty token.TokenType, fn prefixParseFn) {
	p.prefixParseFns[ty] = fn
}
func (p *Parser) registerInfix(ty token.TokenType, fn infixParseFn) {
	p.infixParseFns[ty] = fn
}
...
// expectToken
// if peekToken is t return true and consume current token(p.nextToken())
// if peekToken isn't return false and push a peekError
